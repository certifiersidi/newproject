...
function displayReports($conn){
    $query = mysqli_query($conn, "SELECT * FROM problem ORDER BY date DESC;");
        ...
        foreach($query as $row) :
            echo '<tr><td class="title">'. $row["title"] . "</td>";
            echo '<tr><td class="content">'. $row["content"] . "</td>";
            echo '<tr><td class="email">'. $row["email"] . "</td>";
            echo '<tr><td class="date">'. $row["date"] . "</td>";
            echo '<br><br>';
        endforeach;
